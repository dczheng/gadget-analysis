...
void plot_imshow() {
    ...
    if ( plot_info.log_flag == 1 ){
        ...
        giza_render( PicSize, PicSize, img, 0, PicSize, 0, PicSize,
            log_img_min, log_img_max, 0, affine );
        giza_colour_bar( &cb_s, 1, 3, log_img_min, log_img_max, plot_info.cb_label );
    }
    else {
        giza_render( PicSize, PicSize, img, 0, PicSize, 0, PicSize,
            img_min, img_max, 0, affine );
        giza_colour_bar( &cb_s, 1, 3, img_min, img_max, plot_info.cb_label );
    }
    giza_label( plot_info.xlabel, plot_info.ylabel, plot_info.title );
    fp_tmp = stdout;
    stdout = LogFilefd;
